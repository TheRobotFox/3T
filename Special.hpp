...
#  endif
	...
	struct arg_parser {
		...
		auto operator()() -> std::function<bool(Interpreter &interpreter, ConsExpr sexp, Environment &env,
						Sexp *out)>
		{
			return [*this](Interpreter &interpreter, ConsExpr sexp, Environment &env, Sexp *out){
				std::tuple<Args&...> argv;
	...
	auto read_delimeter_(Interpreter &intp, Environment &env, Sexp *out, const InPortExpr &inp, const CharExpr &c) -> bool;
